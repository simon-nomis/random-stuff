        ...
        if people[person]['mother'] == None and people[person]['father'] == None: 
            ...
            for parent in parents:
                ...
                if parent not in one_gene and parent not in two_genes:
                    ...
                    parents[parent] = 0.5 * (1 - PROBS['mutation'])
                elif parent in two_genes:
                    parents[parent] = 1 - PROBS['mutation']
            if person not in one_gene and person not in two_genes:
                case = 0
                geneprob = (1 - parents[mother]) * (1 - parents[father])
            elif person in one_gene:
                case = 1
                p1 = (1 - parents[mother]) * (parents[father])
                p2 = (parents[mother]) * (1 - parents[father])
                geneprob = p1 + p2
            elif person in two_genes:
                case = 2
                geneprob = (parents[mother]) * (parents[father])
        if person in have_trait:
            traitprob = PROBS['trait'][case][True]
        else:
            traitprob = PROBS['trait'][case][False]
        total *= traitprob * geneprob
    return total
